...
from config import SCRAPED_URLS_FILE, SCRAPED_SERVICES_FILE
...
class ScrapedServiceData(TypedDict):
    """
    TypedDict representing the structure of scraped service data.

    Attributes:
        category (str): The category to which the service belongs.
        service_name (str): The name of the service.
        service_url (str): The URL of the service.
        description (str): A textual description of the service.
        terms (List[str]): A list of terms associated with the service.
        Documents (List[str]): A list of document URLs or identifiers related to the service.
        related_servises (List[RelatedService]): A list of related services, represented by RelatedService objects.
    """
    category: str
    service_name: str
    service_url: str
...
def scrape_all_services():
    """
    Loads service URLs by category, scrapes each service, and saves the results to a JSON file.
    """
    try:
        ...
        with open(SCRAPED_SERVICES_FILE, "w", encoding="utf-8") as f:
